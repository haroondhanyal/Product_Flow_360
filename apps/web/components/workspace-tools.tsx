...
export function WorkspaceTools({requests, onNavigate}: {requests: {id: string; title: string; owner: string}[]; onNavigate: (module: string) => void}) {
  ...
  const [workspaces,setWorkspaces] = useState<{id:string;name:string;description:string;createdAt:string}[]>([]), [activeWorkspace,setActiveWorkspace] = useState(''), [showWorkspaceForm,setShowWorkspaceForm] = useState(false);
  ...
  function createWorkspace(event: React.FormEvent<HTMLFormElement>) {
    event.preventDefault(); const data=new FormData(event.currentTarget); const name=String(data.get('name') ?? '').trim(), description=String(data.get('description') ?? '').trim();
    ...
    try { const workspace={id:crypto.randomUUID(),name,description,createdAt:new Date().toISOString()}; const next=[...workspaces,workspace]; localStorage.setItem('pf360-workspaces',JSON.stringify(next)); localStorage.setItem('pf360-active-workspace',workspace.id); setWorkspaces(next); setActiveWorkspace(workspace.id); setShowWorkspaceForm(false); setMessage(`${name} workspace created and selected.`); } catch { setError('Workspace could not be saved in this browser.'); }
  }
  ...
  return <div className="workspace-tools"><div className="test-intro"><div><span className="eyebrow">FIND, CONNECT, REVIEW</span><h2>Workspace tools</h2><p>Search linked work, import automation results and review change history.</p></div></div>
    ...
    <section className="test-editor workspace-switcher"><div className="section-title"><div><h2>Workspaces</h2><p>Create and switch between separate delivery workspaces.</p></div><button className="primary" onClick={() => setShowWorkspaceForm(open => !open)}>{showWorkspaceForm ? 'Close' : 'New workspace'}</button></div>{showWorkspaceForm && <form className="feature-form" onSubmit={createWorkspace}><label>Workspace name<input name="name" required maxLength={100} autoFocus placeholder="e.g. PTCL Consumer Products"/></label><label>Description <span className="optional">(optional)</span><textarea name="description" rows={2} maxLength={500}/></label><button className="primary" type="submit">Create workspace</button></form>}<div className="workspace-options" role="group" aria-label="Active workspace">{workspaces.map(workspace => <button type="button" key={workspace.id} className={workspace.id===activeWorkspace?'active':''} aria-pressed={workspace.id===activeWorkspace} onClick={() => selectWorkspace(workspace.id)}><strong>{workspace.name}</strong><small>{workspace.description || 'Local delivery workspace'}</small></button>)}</div><p className="report-note">The selected workspace is persisted in this browser. The new API enforces workspace boundaries; the existing local prototype data is shared until each screen completes its API migration.</p></section>
