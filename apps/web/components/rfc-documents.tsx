...
export function RfcDocuments({ requestId, evidence = false, title = 'RFC documents' }: { requestId: string; evidence?: boolean; title?: string }) {
  ...
  return <section className="rfc-documents" aria-label={title}>
    ...
    <ul className="document-list">{documents.map(info => <li key={info.id}>
      ...
      {evidenceMime(info.name) && <button type="button" className="icon-button" title="View attached evidence" aria-label={`View ${info.name}`} onClick={() => { setError(''); void previewDocument(info.id).then(blob => setPreview({url: URL.createObjectURL(blob), type: blob.type, name: info.name})).catch(error => setError(documentError(error))); }}><Eye size={17}/></button>}
