...
import { AUTH_SESSION_KEY, type AuthUser, ensureDemoAdmin, loadDepartments, loadUsers, passwordHash, saveUsers } from '../lib/auth';
...
function PasswordInput({name, label, visible, setVisible, confirm = false, login = false}: {name: string; label: string; visible: boolean; setVisible: (value: boolean) => void; confirm?: boolean; login?: boolean}) {
  return <label>{label}<span className="password-field"><input name={name} type={visible ? 'text' : 'password'} required minLength={12} autoComplete={confirm ? 'new-password' : login ? 'current-password' : 'new-password'} placeholder={confirm ? 'Re-enter your password' : login ? 'Enter your password' : 'At least 12 characters'}/><button type="button" aria-label={visible ? `Hide ${label}` : `Show ${label}`} onClick={() => setVisible(!visible)}>{visible ? <EyeOff size={17}/> : <Eye size={17}/>}</button></span></label>;
}
export function AuthPortal({children}: {children: (session: Session) => ReactNode}) {
  const [ready, setReady] = useState(false), [user, setUser] = useState<AuthUser | null>(null), [mode, setMode] = useState<'login' | 'signup'>('login');
  const [departments, setDepartments] = useState<string[]>([]), [error, setError] = useState(''), [notice, setNotice] = useState(''), [busy, setBusy] = useState(false), [photo, setPhoto] = useState(''), [showLoginPassword, setShowLoginPassword] = useState(false), [showSignupPassword, setShowSignupPassword] = useState(false), [showConfirmPassword, setShowConfirmPassword] = useState(false);
  ...
  async function login(event: React.FormEvent<HTMLFormElement>) {
    event.preventDefault(); setBusy(true); setError(''); setNotice(''); const form = new FormData(event.currentTarget); const email = String(form.get('email')).trim().toLowerCase(); const password = String(form.get('password'));
  ...
  if (!ready) return <div className="auth-loading">Loading secure workspace…</div>;
