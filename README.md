...
### RFC documents
Open any request (newly created requests open automatically), then choose or drop documents in **RFC documents**. PDF, Word, Excel, CSV, image, video, JSON, TXT and LOG files are supported, up to **50 MB per file (50 × 1024 × 1024 bytes)**. Attachments support image/video preview, download, confirmed removal and persistent IndexedDB storage, separately from localStorage. The same evidence control is available in Requirements, Defects, UAT approvals, Test Cases and each Test Run. Validation checks the extension, size, empty files and basic format signatures where applicable; it does not parse document contents or scan for malware. Browser quota failures display an error without creating a saved attachment.
