...
if (run('node', ['playwright/scripts/reset-allure-results.mjs', '--fresh']) !== 0) process.exit(1);
let server;
let failed = false;
try {
  let ready = false;
  for (let attempt = 0; attempt < 60; attempt++) {
    try {
      ...
      if (!server) server = spawn('node', ['../../node_modules/next/dist/bin/next', 'start', '--hostname', '127.0.0.1', '--port', '3100'], { cwd: 'apps/web', stdio: 'inherit' });
    }
